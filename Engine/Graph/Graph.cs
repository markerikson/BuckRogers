...
namespace skmDataStructures.Graph
{
	...
	public class Graph
	{
		...
		#region Removing Edge Methods

		/// <summary>
		/// Removes a directed, weighted edge from one node to another.
		/// </summary>
		/// <param name="uKey">The <b>Key</b> of the node from which the directed edge emanates.</param>
		/// <param name="vKey">The <b>Key</b> of the node from which the directed edge leads to.</param>
		/// <remarks>If nodes with <b>uKey</b> and <b>vKey</b> do not exist in the graph, an <b>ArgumentException</b>
		/// exception is thrown.</remarks>
		public virtual void RemoveDirectedEdge(string uKey, string vKey)
		{
			// get references to uKey and vKey
			if (nodes.ContainsKey(uKey) && nodes.ContainsKey(vKey))
				RemoveDirectedEdge(nodes[uKey], nodes[vKey]);
			else
				throw new ArgumentException("One or both of the nodes supplied were not members of the graph.");
		}
		...
		/// Removes a directed, weighted edge from one node to another.
		/// </summary>
		/// <param name="u">The node from which the directed edge emanates.</param>
		/// <param name="v">The node from which the directed edge leads to.</param>
		/// <param name="cost">The weight of the edge.</param>
		/// <remarks>If the passed-in nodes do not exist in the graph, an <b>ArgumentException</b>
		/// exception is thrown.</remarks>
		public virtual void RemoveDirectedEdge(Node u, Node v)
		{
			// Make sure u and v are Nodes in this graph
			if (nodes.ContainsKey(u.Key) && nodes.ContainsKey(v.Key))
				// Remove an edge from u -> v
				u.RemoveDirected(v);
			else			
				// one or both of the nodes were not found in the graph
				throw new ArgumentException("One or both of the nodes supplied were not members of the graph.");
		...
		/// <summary>
		/// Removes an undirected, weighted edge from one node to another.
		/// </summary>
		/// <param name="cost">The weight of the edge.</param>
		/// <remarks>If nodes with <b>uKey</b> and <b>vKey</b> do not exist in the graph, an <b>ArgumentException</b>
		/// exception is thrown.</remarks>
		public virtual void RemoveUndirectedEdge(string uKey, string vKey)
		{
			// get references to uKey and vKey
			if (nodes.ContainsKey(uKey) && nodes.ContainsKey(vKey))
				RemoveUndirectedEdge(nodes[uKey], nodes[vKey]);
			else
				throw new ArgumentException("One or both of the nodes supplied were not members of the graph.");
		}
		...
		/// <remarks>If the passed-in nodes do not exist in the graph, an <b>ArgumentException</b>
		/// exception is thrown.</remarks>
		public virtual void RemoveUndirectedEdge(Node u, Node v)
		{
			// Make sure u and v are Nodes in this graph
			if (nodes.ContainsKey(u.Key) && nodes.ContainsKey(v.Key))
			{
				// Remove an edge from u -> v and from v -> u
				u.RemoveDirected(v);
				v.RemoveDirected(u);
			}
			else			
		...
		public ArrayList ShortestPath(Node start, Node dest)
		{
			...
			while(queue.Count > 0)
			{
				...
				if(node == dest)
				{
					return ConstructPath(node, parents);
				}
				else
				{
					visited.Add(node);
		...
		private ArrayList ConstructPath(Node dest, Hashtable parents)
		{
			ArrayList al = new ArrayList();
			while(parents.ContainsKey(dest))
			{
				al.Insert(0, dest);
				dest = (Node)parents[dest];
			}
			return al;
		}
